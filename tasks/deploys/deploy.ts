...
task("contraction-configuration", " Init the proxy contracts")
  .setAction(async ( taskArgs , hre) => {
    ...
    await aLendPoolConfiguratorProxy.configureNftAsCollateral(nftAssets, 5000, 7000, 1000, 500);
...
task("get-reserve-data", "Get contract address from address provider")
  .setAction(async ( taskArgs,hre ) => {
    ...
    const reserveData = await lendPool.getReserveData(jsonData.mockWETHAddress);
