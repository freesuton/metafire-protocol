...
import {MetaFireUpgradeableProxy} from "../libraries/proxy/MetaFireUpgradeableProxy.sol";
...
contract LendPoolAddressesProvider is Ownable, ILendPoolAddressesProvider {
  ...
  /**
   * @dev Returns the id of the MetaFire market to which this contracts points to
  ...
  function _updateImpl(bytes32 id, address newAddress) internal {
    ...
    if (proxyAddress == address(0)) {
      ...
      MetaFireUpgradeableProxy proxy = new MetaFireUpgradeableProxy(newAddress, address(this), params);

      _addresses[id] = address(proxy);
      emit ProxyCreated(id, address(proxy));
    } else {
      // upgrade implementation
      MetaFireUpgradeableProxy proxy = MetaFireUpgradeableProxy(proxyAddress);
